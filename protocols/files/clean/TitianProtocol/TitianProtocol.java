// ***This version is not to be distributed***
// TitianProtocol
//
...
public class TitianProtocol implements Runnable {
	...
	public TitianProtocol(Socket myConnection) {
	...
	public void run() {
		...
		try {
			...
			byte[] b2 = "Connect Protocol 1".getBytes();
			String s2 = new String(message1);
			...
			try {
				...
				// Protocol Step 6
				byte[] plainTextM6 = ("Well Done. Submit this value: " + token).getBytes();
	...
	public static void main(String[] args) {
		...
		try (ServerSocket server = new ServerSocket(portNo)) {
			System.out.println("TitianProtocol running on port " + server.getLocalPort());
			while (true) {
				// For each connection spin off a new protocol instance.
				Socket connection = server.accept();
				Thread instance = new Thread(new TitianProtocol(connection));
				instance.start();
			}
		} catch (Exception e) {
